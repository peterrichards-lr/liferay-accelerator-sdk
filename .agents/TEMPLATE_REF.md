...
- [#116](https://github.com/peterrichards-lr/liferay-accelerator-sdk/issues/116) — PR #105 edited real content in 5 skill files without bumping their required `Last Reviewed` footers, despite `.agents/skills/documentation/SKILL.md` mandating it. `ai-agent-template` fixed the underlying cause of this class of bug in its PR #16 (append-only timestamp injection instead of rewrite-on-touch, removing the dependency on an agent remembering to bump a date).
