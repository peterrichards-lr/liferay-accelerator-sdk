...
- **Permission Exceptions**: If a `gh` command fails with a permission error requesting a specific grant (e.g. `To allow this command, you likely need grant: gh.update(...)`), you MUST immediately halt. Claude Code's permission system already prompts the user automatically when a tool call requires a grant it doesn't have — do not invent or attempt to invoke a separate permission-granting tool. Await the user's response to that prompt before proceeding.
